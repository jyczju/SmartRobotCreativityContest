...
import numpy as np
import os
from MSER_NMS import rgb2hsv
from color_detect import color_hsv_detect
...
def extract_green(img, color_hsv):
    '''提取棋子区域'''
    src = None
    sourceImage = img.copy()
    qizi_Hanzi = None
    ...
    lower_green = np.array([color_hsv[0], color_hsv[1]-40, color_hsv[2]-40])  # 设定绿色的阈值下限
    upper_green = np.array([color_hsv[0]+20, color_hsv[1]+60, color_hsv[2]+60])  # 设定绿色的阈值上限
