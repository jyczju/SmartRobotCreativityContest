...
SoftwareSerial mySerial(22, 23); //使用软件串口，模块TX接D22，RX接D23
Servo servo1;                    //定义Servo对象来控制
Servo servo2;                    //定义Servo对象来控制
int left_angle = 45;             //向左倾斜对应的舵机角度
int right_angle = 135;           //向右倾斜对应的舵机角度
...
void setup()
{
    ...
    Serco_Syn_Write(origin_angle);     //舵机角度初始化
    delay(DELAY_TIME);
}
...
{   
    ...
    if (Serial.available())
    {
        ...
        if (inputChar == 'A')
        {
            ...
            Serco_Syn_Write(left_angle); //向左倾斜
            delay(DELAY_TIME);
        ...
        else if (inputChar == 'B')
        {
            ...
            Serco_Syn_Write(right_angle); //向右倾斜
            delay(DELAY_TIME);
            Serco_Syn_Write(origin_angle); //回到初始角度
            delay(DELAY_TIME);
        }
